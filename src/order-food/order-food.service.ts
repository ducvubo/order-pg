...
export class OrderFoodService {
  ...
  async createOrderFood(createOrderFoodDto: CreateOrderFoodDto, id_user_guest: string): Promise<OrderFoodEntity> {
    ...
    try {
      ...
      const {
        od_res_id, od_user_address, od_user_district, od_user_email, od_user_name, od_user_note, od_user_phone, od_user_province, od_user_ward, od_user_id,
        od_price_shipping, od_type_shipping, order_food_items, od_link_confirm
      } = createOrderFoodDto
      ...
      await Promise.all(order_food_items.map(async (item) => {
        ...
        if (currentTime < openTime || currentTime > closeTime) {
          throw new BadRequestError('Món ăn này không trong thời gian phục vụ hiện tại của nhà hàng, vui lòng thử lại sau');
        }

        // Kiểm tra tùy chọn món ăn (nếu có)
        let listFoodOption: FoodOptionsEntity[] = []
        if (item.food_options?.length) {
          await Promise.all(item.food_options.map(async (option) => {
